...
static const char* token_type_names[] = {
    "NUMBER", "STRING", "SET", "TO", "IF", "FOR", "WHILE", "IN", "RANGE",
    "FUNCTION", "WITH", "CALL", "RETURN", "IS", "EQUAL", "NOT", "GREATER",
    "LESS", "THAN", "AND", "OR", "PRINT", "PLUS", "MINUS", "TIMES", "DIVIDED",
...
static void tokenize_line(TokenArray* arr, const char* line, int indent) {
    ...
    while (col < len) {
        ...
        if (line[col] == '"') {
            ...
            if (col < len) col++; // Skip closing quote
...
TokenArray* tokenize(const char* source) {
    ...
    while (*line_start) {
        ...
        // Calculate indent
        int indent = 0;
        for (size_t i = 0; i < line_len && (line_start[i] == ' ' || line_start[i] == '\t'); i++) {
            indent++;
        }
