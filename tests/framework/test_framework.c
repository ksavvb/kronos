...
void test_register(const char *name, TestFunc func, const char *file) {
  ...
  if (g_test_case_count >= g_test_case_capacity) {
    size_t new_capacity =
        g_test_case_capacity == 0 ? 16 : g_test_case_capacity * 2;
    TestCase *new_cases =
        realloc(g_test_cases, new_capacity * sizeof(TestCase));
    if (new_cases == NULL) {
...
void test_fail(const char *file, int line, const char *message) {
  ...
  const char *test_name =
      g_current_test_name != NULL ? g_current_test_name : "unknown";
  ...
  if (g_test_suite->count >= g_test_suite->capacity) {
    size_t new_capacity =
        g_test_suite->capacity == 0 ? 16 : g_test_suite->capacity * 2;
    TestResult *new_results =
        realloc(g_test_suite->results, new_capacity * sizeof(TestResult));
    if (new_results == NULL) {
...
void test_run_all(void) {
  ...
  for (size_t i = 0; i < g_test_case_count; i++) {
    current_test++;
    printf("[%zu/%zu] Running %s... ", current_test, total_tests,
           g_test_cases[i].name);
    fflush(stdout);
  ...
  if (g_test_suite->failed > 0) {
    ...
    for (size_t i = 0; i < g_test_suite->count; i++) {
      if (!g_test_suite->results[i].passed) {
        ...
        printf("  %s:%d\n", g_test_suite->results[i].file,
               g_test_suite->results[i].line);
        if (g_test_suite->results[i].message != NULL) {
